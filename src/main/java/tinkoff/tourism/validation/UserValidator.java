...
public class UserValidator implements ConstraintValidator<UserConstraint, User> {
    ...
    public static final Pattern validName = Pattern.compile("^([a-zA-Zа-яА-Я]{2,}\\s[a-zA-Zа-яА-Я]+'?-?[a-zA-Zа-яА-Я]{2,}\\s?([a-zA-Zа-яА-Я]+)?)");
    // Valid names examples: John Doe, John D'oe or Hector Senior-Json

    public static final Pattern validLogin = Pattern.compile("^[a-zA-Z\\d]([._-](?![._-])|[a-zA-Z\\d]){3,64}[a-zA-Z\\d]$");
